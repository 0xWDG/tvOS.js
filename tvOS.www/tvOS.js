...
var tvOS = {
   ...
   * @param function [callbackOnSearch] The callback function when searching
   * @param function [callbackOnSelect] The callback function when clicking on a 'item'
   ...
   * @example })
   */
  searchView: function (search, results, items, callbackOnSearch, callbackOnSelect) {
    var temp = ''
    ...
    //
    // On text change callbackOnSearch(myKeyboard.text)
    myKeyboard.onTextChange = function () {
      // Is there a callback?
      if (typeof callbackOnSearch === 'function') {
        // Trigger the callback
        callbackOnSearch(myKeyboard.text)
      } else {
  ...
  loadYouTubeVideo: function (videoID, callback) {
    ...
    tvOS.ajax('http://www.youtube.com/get_video_info?video_id=' + videoID + '&asv=2', 'GET', function (data) {
      ...
      //
      // realStream (temporary) array
      var realStream = {}
      // parse this string into a array
      tvOS.parse_str(streams[i], realStream)
      // ok, magic
      realStream['url'] += '&signature=' + realStream['sig']
      // push it
   ...
   * @param function [callbackRegister] register callback action
   ...
   */
  login: function (title, callback, text, image, callbackRegister, register, needPWD) {
    if (typeof callback === 'function') {
      ...
      if (typeof callbackRegister === 'function' && typeof register === 'string') {
      ...
      ttemp.addEventListener('select', function (e) {
        ...
        if (pressed === text) {
          ...
          callbackRegister(inputText)
        } else {
          // Clicked on our 'Next' button!
          tvOS.loginText = inputText
          tvOS.login(title, callback, text, image, callbackRegister, register, true) // Login? :D
        }
      })
